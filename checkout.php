...
try{
	...
	foreach($all_carts as $cart){
		...
		$quantity[$cart['book_id']]=$cart['quantity'];
	...
	if(isset($_POST['placeorder'])){
		...
		foreach($all_books as $book){
			...
			$q->execute([$book['quantity']-$quantity[$book['id']],$book['id']]);
			$q=$db->prepare("INSERT INTO orders 
							VALUES(NULL,?,?,?,?,?,NULL,0);");
			$q->execute([$book['id'],$customer_id,$book['price'],$quantity[$book['id']],date('Y-m-d')]);
		}
	...
	<div class="row">
		<div class="col-md-6 p-2 my-5">
			...
			<?php foreach($all_books as $book){
					$total_price+=$book['price']*$quantity[$book['id']]; ?>
  				<li class="list-group-item d-flex justify-content-between align-items-center">
    				<?php echo $book['title']."(".$book['language'].")"; ?>
    				<span class="badge bg-primary rounded-pill">
    					<?php echo $quantity[$book['id']]; ?>
    				</span>
  				</li>
  			<?php } ?>
			</ul>	
